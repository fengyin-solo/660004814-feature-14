...
import type { Conformation, SamplingResult, ProteinParams, BatchRecord } from '@/types'
...
export const useProteinStore = defineStore('protein', () => {
  ...
  const history = ref<BatchRecord[]>([])
  let batchSeq = 0
  // 选中批次统一走 Set 去重：重复勾选/整组提交都不会把同一批算两遍
  const selectedBatchIds = ref<string[]>(dedupeBatchIds(loadSelectedIds()))

  const selectedBatches = computed(() => {
    const map = new Map(history.value.map(b => [b.id, b]))
    return selectedBatchIds.value.map(id => map.get(id)).filter((b): b is BatchRecord => !!b)
  })
  ...
  function setSelectedBatches(ids: string[]) {
